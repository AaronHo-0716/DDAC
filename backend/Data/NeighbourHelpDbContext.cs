...
public partial class NeighbourHelpDbContext : DbContext
{
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        modelBuilder.Entity<Conversation>(entity =>
        {
            ...
            entity.Property(e => e.Type).HasColumnName("type")
                .HasConversion(v => v.ToLower().Trim(), v => v.ToLower().Trim());
        ...
        modelBuilder.Entity<Message>(entity =>
        {
            ...
            entity.Property(e => e.Message_Type).HasColumnName("message_type")
                .HasConversion(v => v.ToLower().Trim(), v => v.ToLower().Trim());
                
            entity.Property(e => e.Body_Text).HasColumnName("body_text");
            entity.Property(e => e.Created_At_Utc).HasColumnName("created_at_utc").HasDefaultValueSql("now()");

            entity.HasOne(d => d.Conversation).WithMany(p => p.Messages).HasForeignKey(d => d.Conversation_Id);
            entity.HasOne(d => d.Sender_User).WithMany().HasForeignKey(d => d.Sender_User_Id);
        });
        ...
        modelBuilder.Entity<Conversation_Participant>(entity =>
        {
            ...
            entity.HasOne(d => d.Conversation).WithMany(p => p.Participants).HasForeignKey(d => d.Conversation_Id);
            entity.HasOne(d => d.User).WithMany().HasForeignKey(d => d.User_Id);
            
            // Ensure a user can only be in a conversation once
            entity.HasIndex(e => new { e.Conversation_Id, e.User_Id }).IsUnique();
        });
