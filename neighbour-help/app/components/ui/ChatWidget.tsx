...
export default function ChatWidget() {
  ...
  const loadConversations = useCallback(async () => {
    ...
    try {
      ...
      const unique = Array.from(
        new Map([...jobList, ...supportList].map((c) => [c.id, c])).values(),
      );
      setConversations(
        unique.sort((a, b) => {
          const timeA = a.lastMessageAtUtc
            ? new Date(a.lastMessageAtUtc).getTime()
            : 0;
          const timeB = b.lastMessageAtUtc
  ...
  useEffect(() => {
    ...
    connection.on("ReceiveMessage", (payload: ChatHubMessagePayload) => {
      ...
      if (activeConversationIdRef.current === conversationId) {
        ...
        const convType = conversationsRef.current.find(
          (c) => c.id === conversationId,
        )?.type;
        if (convType === ConversationType.AdminSupport)
      ...
      setConversations((prev) => {
        ...
        const isParticipant = target.participants.some(
          (p) => p.userId === user.id,
        );
        const isUnassignedSupport =
          target.type === ConversationType.AdminSupport &&
          !target.participants.some((p) => p.role.toLowerCase() === "admin");

        const shouldMarkUnread =
          (isParticipant && incoming.senderId !== user.id) ||
          (isAdmin && isUnassignedSupport);
    ...
    connection.on(
      "SupportChatTaken",
      (payload: { conversationId: string; assignedAdminId: string }) => {
  ...
  useEffect(() => {
    if (isOpen && activeConversationId && user)
  ...
  const handleSend = async () => {
    ...
    try {
      if (hasImage && selectedImage) {
        ...
        if (isSupport)
          await uploadsService.uploadSupportChatAttachmentImage(
            selectedImage,
            activeConversationId,
          );
        else
          await uploadsService.uploadJobChatAttachmentImage(
            selectedImage,
            activeConversationId,
          );
        resetSelectedImage();
      ...
      if (hasText) {
        if (isSupport)
          await supportService.sendMessage(activeConversationId, {
            content: value,
            messageType: "text",
          });
        else
  ...
  useEffect(() => {
    ...
    const initChat = async () => {
      ...
      try {
        const c = await messagesService.createJobConversation({
  ...
  return (
    <>
      <button
        onClick={open}
        className="fixed bottom-6 right-6 z-40 w-12 h-12 rounded-full bg-[#0B74FF] text-white shadow-lg flex items-center justify-center hover:bg-[#065ed1] transition-colors"
      >
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          ...
                          <p
                            className={`text-[10px] mt-1 ${mine ? "text-blue-100" : "text-[#9CA3AF]"}`}
                          >
